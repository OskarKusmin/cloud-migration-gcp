...
resource "google_service_account_iam_member" "external_secrets_wi" {
  ...
  member = "serviceAccount:${var.project_id}.svc.id.goog[external-secrets/prod-external-secrets]"
...
resource "google_service_account_iam_member" "external_dns_wi" {
  ...
  role = "roles/iam.workloadIdentityUser"
  member = "serviceAccount:${var.project_id}.svc.id.goog[external-dns/prod-external-dns]"
  depends_on = [module.gke]
}
...
resource "google_service_account_iam_member" "grafana_wi" {
  ...
  role = "roles/iam.workloadIdentityUser"
  member = "serviceAccount:${var.project_id}.svc.id.goog[monitoring/prod-kube-prometheus-stack-grafana]"
  depends_on = [module.gke]
}
